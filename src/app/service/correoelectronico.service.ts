...
import { URL_ENVIAR_CORREO_ELECTRONICO } from 'app/utils/app.constants';
...
export class CorreoElectronicoService {
    ...
    enviar(direcciones, asunto:string, mensaje:string) {
        let headers = this.getHeaders();
        let options = new RequestOptions({ headers: headers });

        for (var i = 0; i < direcciones.length; i++) {
            let msg=JSON.stringify({ direccion: direcciones[i],mensaje: mensaje, asunto: asunto});
            this.http.post(URL_ENVIAR_CORREO_ELECTRONICO, msg, options).catch(this.handleError).subscribe();
        }
    }
