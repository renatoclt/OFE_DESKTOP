...
import { Cotizacion, Atributo, Producto, Archivo } from 'app/model/egp-cotizacion';
...
@Component({
  ...
  providers: [CorreoElectronicoService, MensajeTextoService]
})
...
export class CotizacionProveedorFormularioComponent implements OnInit, OnChanges, AfterViewInit {
  ...
  constructor(activatedRoute: ActivatedRoute,private  router:Router, private _emailService: CorreoElectronicoService, private _smsService: MensajeTextoService) {
  ...
  enviar(e) {
    ...
    let emailAsunto = `Nueva cotización de Pintado de Edificio S.A.`;

    //let emailContenido = "mensaje de prueba - contenido";
    let emailContenido = `Estimados Sres.
    
    La presente es para comunicarles que Pintado de Edificio S.A. les ha hecho llegar una cotización en respuesta al requerimiento REQ001
    ...
    let smsContenido = `Estimado cliente

Usted ha recibido una cotización de Pintado de Edificio S.A..

Atentamente,

Pintado de Edificio S.A.`;




    this._emailService.enviar(['jrojas@ebizlatin.com'],emailAsunto,emailContenido); 
    this._smsService.enviar(['945119162'],smsContenido); 

    swal({
      text: "La cotización fue enviada correctamente!",
      type: 'success',
      buttonsStyling: false,
      confirmButtonClass: "btn btn-success"
    }).then(function () {
      this.router.navigate(['/egp-requerimiento/proveedor/buscar']);
    });
  ...
  ngOnInit() {
    ...
    this.item = {
      ...
      atenciona: "Usuario PROVIAS",
      preparadopor: "Pintado de Edificio S.A",
      moneda: "SOLES",
      mensaje: "",
      productos: [
        {
          ...
          cantidad: "",
          precio: "",
          adjuntos:'TDRPintado de Edificio Institucional',
          fechaentrega: '19/02/2018',
          atributos: [
            {
              nombre: 'F. Entrega',
              operador: '=',
...
function CargarDataTableArchivos() {
    ...
    columnDefs: [
      ...
      },
      {
        ...
        render: function (data, type, row) {
          ...
          return '<div class="text-center"><a class="editar" href="/assets/documentos/cielo.jpeg" row-id="' + row.id + '">' +
  ...
  dtArchivos.on('click', '.download', function (event) {
    ...
    let row_id = $tr.find("a.editar").attr('href');
