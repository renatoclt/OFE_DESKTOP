...
var oFacturaProveedorBuscarComponent;
var datatable;
@Component({
  moduleId: module.id,
...
export class FacturaProveedorBuscarComponent implements OnInit, AfterViewInit {
    ...
    if (botones) {
      ...
      this._securityService.obtenerBotones(this.url_main_module_page).subscribe(
        botones => {
          ...
          oFacturaProveedorBuscarComponent.configurarBotones(botones);
          oFacturaProveedorBuscarComponent._securityService.guardarBotonesLocalStore(this.url_main_module_page, botones);
        },
        e => console.log(e),
        () => { });
  ...
  validarfiltros() {
    ...
    if (this.filtro.fechacreacioninicio != null && this.filtro.fechacreacioninicio.toString() != "" && this.filtro.fechacreacionfin != null && this.filtro.fechacreacionfin.toString() != "") {
      let fechacreacioninicio = DatatableFunctions.ConvertStringToDatetime(oFacturaProveedorBuscarComponent.filtro.fechacreacioninicio);
      let fechacreacionfin = DatatableFunctions.ConvertStringToDatetime(oFacturaProveedorBuscarComponent.filtro.fechacreacionfin);
  ...
  ngOnInit() {
    ...
    oFacturaProveedorBuscarComponent = this;

    this.util.listEstadoOC(function (data: ComboItem[]) {
      oFacturaProveedorBuscarComponent.listEstadoCombo = data;
    });
    this.filtroDefecto();
...
function filtrarResultados(item) {
  ...
  let nroordencomprafiltro = oFacturaProveedorBuscarComponent.filtro.nroordencompra as string;
...
function cargarDataTable() {
  ...
  datatable = $('#dtResultados').on('init.dt', function (e, settings, json) {
    ...
    ajax: {
      ...
      beforeSend: function (request) {
        if (!oFacturaProveedorBuscarComponent.util.tokenValid()) {
          return;
        };
      ...
      data: function (d) {
        ...
        if (oFacturaProveedorBuscarComponent.filtro.ruc != "") {
          d.ruc = oFacturaProveedorBuscarComponent.filtro.ruc.trim();
        }

        if (oFacturaProveedorBuscarComponent.filtro.razonsocial != "") {
          d.razonsocial = oFacturaProveedorBuscarComponent.filtro.razonsocial.trim();

        }



        if (oFacturaProveedorBuscarComponent.filtro.fechacreacioninicio) {

          let fechacreacioninicio = DatatableFunctions.ConvertStringToDatetime(oFacturaProveedorBuscarComponent.filtro.fechacreacioninicio);
          d.FechaRegistroInicio = DatatableFunctions.FormatDatetimeForMicroService(fechacreacioninicio);
        }

        if (oFacturaProveedorBuscarComponent.filtro.fechacreacionfin) {

          let fechacreacionfin = DatatableFunctions.AddDayEndDatetime(DatatableFunctions.ConvertStringToDatetime(oFacturaProveedorBuscarComponent.filtro.fechacreacionfin));
          d.FechaRegistroFin = DatatableFunctions.FormatDatetimeForMicroService(fechacreacionfin);
        }

        let tipos_oc = [];

        if (oFacturaProveedorBuscarComponent.filtro.material)
          tipos_oc.push('M');
        if (oFacturaProveedorBuscarComponent.filtro.servicio)
          tipos_oc.push('S');
  ...
  datatable.on('click', '.edit', function (event) {
    if (oFacturaProveedorBuscarComponent.botonDetalle.habilitado) {
      ...
      //console.log("click edit", oFacturaProveedorBuscarComponent);
      let nav = ['/ordencompra/proveedor/formulario', nroordencompra];

      oFacturaProveedorBuscarComponent.navigate(nav);
    }
    event.preventDefault();
