...
var oGuiaProveedorBuscarComponent;
var datatable;
@Component({
  moduleId: module.id,
...
export class GuiaProveedorBuscarComponent implements OnInit, AfterViewInit {
    ...
    if (botones) {
      ...
      this._securityService.obtenerBotones(this.url_main_module_page).subscribe(
        botones => {
          ...
          oGuiaProveedorBuscarComponent.configurarBotones(botones);
          oGuiaProveedorBuscarComponent._securityService.guardarBotonesLocalStore(this.url_main_module_page, botones);
        },
        e => console.log(e),
        () => { });
  ...
  validarfiltros() {
    ...
    if (this.filtro.fechacreacioninicio != null && this.filtro.fechacreacioninicio.toString() != "" && this.filtro.fechacreacionfin != null && this.filtro.fechacreacionfin.toString() != "") {
      let fechacreacioninicio = DatatableFunctions.ConvertStringToDatetime(oGuiaProveedorBuscarComponent.filtro.fechacreacioninicio);
      let fechacreacionfin = DatatableFunctions.ConvertStringToDatetime(oGuiaProveedorBuscarComponent.filtro.fechacreacionfin);
  ...
  ngOnInit() {
    ...
    oGuiaProveedorBuscarComponent = this;

    this.util.listEstadoOC(function (data: ComboItem[]) {
      oGuiaProveedorBuscarComponent.listEstadoCombo = data;
    });
    this.filtroDefecto();
...
function filtrarResultados(item) {
  ...
  let nroordencomprafiltro = oGuiaProveedorBuscarComponent.filtro.nroordencompra as string;
...
function cargarDataTable() {
  ...
  datatable = $('#dtResultados').on('init.dt', function (e, settings, json) {
    ...
    ajax: {
      ...
      beforeSend: function (request) {
        if (!oGuiaProveedorBuscarComponent.util.tokenValid()) {
          return;
        };
      ...
      data: function (d) {
        ...
        if (oGuiaProveedorBuscarComponent.filtro.ruc != "") {
          d.ruc = oGuiaProveedorBuscarComponent.filtro.ruc.trim();
        }

        if (oGuiaProveedorBuscarComponent.filtro.razonsocial != "") {
          d.razonsocial = oGuiaProveedorBuscarComponent.filtro.razonsocial.trim();

        }



        if (oGuiaProveedorBuscarComponent.filtro.fechacreacioninicio) {

          let fechacreacioninicio = DatatableFunctions.ConvertStringToDatetime(oGuiaProveedorBuscarComponent.filtro.fechacreacioninicio);
          d.FechaRegistroInicio = DatatableFunctions.FormatDatetimeForMicroService(fechacreacioninicio);
        }

        if (oGuiaProveedorBuscarComponent.filtro.fechacreacionfin) {

          let fechacreacionfin = DatatableFunctions.AddDayEndDatetime(DatatableFunctions.ConvertStringToDatetime(oGuiaProveedorBuscarComponent.filtro.fechacreacionfin));
          d.FechaRegistroFin = DatatableFunctions.FormatDatetimeForMicroService(fechacreacionfin);
        }

        let tipos_oc = [];

        if (oGuiaProveedorBuscarComponent.filtro.material)
          tipos_oc.push('M');
        if (oGuiaProveedorBuscarComponent.filtro.servicio)
          tipos_oc.push('S');
  ...
  datatable.on('click', '.edit', function (event) {
    if (oGuiaProveedorBuscarComponent.botonDetalle.habilitado) {
      ...
      //console.log("click edit", oGuiaProveedorBuscarComponent);
      let nav = ['/ordencompra/proveedor/formulario', nroordencompra];

      oGuiaProveedorBuscarComponent.navigate(nav);
    }
    event.preventDefault();
