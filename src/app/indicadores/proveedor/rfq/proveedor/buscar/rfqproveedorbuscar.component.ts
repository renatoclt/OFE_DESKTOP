...
var oRfqProveedorBuscarComponent;
var datatable;
@Component({
  moduleId: module.id,
  selector: 'rfqproveedorbuscar-cmp',
  templateUrl: 'rfqproveedorbuscar.component.html',
  providers: [MasterService, LoginService]
})

export class RfqProveedorBuscarComponent implements OnInit, AfterViewInit {
  util: AppUtils;
    ...
    if (botones) {
      ...
      this._securityService.obtenerBotones(this.url_main_module_page).subscribe(
        botones => {
          ...
          oRfqProveedorBuscarComponent.configurarBotones(botones);
          oRfqProveedorBuscarComponent._securityService.guardarBotonesLocalStore(this.url_main_module_page, botones);
        },
        e => console.log(e),
        () => { });
  ...
  validarfiltros() {
    ...
    if (this.filtro.fechacreacioninicio != null && this.filtro.fechacreacioninicio.toString() != "" && this.filtro.fechacreacionfin != null && this.filtro.fechacreacionfin.toString() != "") {
      let fechacreacioninicio = DatatableFunctions.ConvertStringToDatetime(oRfqProveedorBuscarComponent.filtro.fechacreacioninicio);
      let fechacreacionfin = DatatableFunctions.ConvertStringToDatetime(oRfqProveedorBuscarComponent.filtro.fechacreacionfin);
  ...
  ngOnInit() {
    ...
    oRfqProveedorBuscarComponent = this;

    this.util.listEstadoOC(function (data: ComboItem[]) {
      oRfqProveedorBuscarComponent.listEstadoCombo = data;
    });
    this.filtroDefecto();
...
function filtrarResultados(item) {
  ...
  let nroordencomprafiltro = oRfqProveedorBuscarComponent.filtro.nroordencompra as string;
...
function cargarDataTable() {
  ...
  datatable = $('#dtResultados').on('init.dt', function (e, settings, json) {
    ...
    ajax: {
      ...
      beforeSend: function (request) {
        if (!oRfqProveedorBuscarComponent.util.tokenValid()) {
          return;
        };
      ...
      data: function (d) {
        ...
        if (oRfqProveedorBuscarComponent.filtro.ruc != "") {
          d.ruc = oRfqProveedorBuscarComponent.filtro.ruc.trim();
        }

        if (oRfqProveedorBuscarComponent.filtro.razonsocial != "") {
          d.razonsocial = oRfqProveedorBuscarComponent.filtro.razonsocial.trim();

        }



        if (oRfqProveedorBuscarComponent.filtro.fechacreacioninicio) {

          let fechacreacioninicio = DatatableFunctions.ConvertStringToDatetime(oRfqProveedorBuscarComponent.filtro.fechacreacioninicio);
          d.FechaRegistroInicio = DatatableFunctions.FormatDatetimeForMicroService(fechacreacioninicio);
        }

        if (oRfqProveedorBuscarComponent.filtro.fechacreacionfin) {

          let fechacreacionfin = DatatableFunctions.AddDayEndDatetime(DatatableFunctions.ConvertStringToDatetime(oRfqProveedorBuscarComponent.filtro.fechacreacionfin));
          d.FechaRegistroFin = DatatableFunctions.FormatDatetimeForMicroService(fechacreacionfin);
        }

        let tipos_oc = [];

        if (oRfqProveedorBuscarComponent.filtro.material)
          tipos_oc.push('M');
        if (oRfqProveedorBuscarComponent.filtro.servicio)
          tipos_oc.push('S');
  ...
  datatable.on('click', '.edit', function (event) {
    if (oRfqProveedorBuscarComponent.botonDetalle.habilitado) {
      ...
      //console.log("click edit", oRfqProveedorBuscarComponent);
      let nav = ['/ordencompra/proveedor/formulario', nroordencompra];

      oRfqProveedorBuscarComponent.navigate(nav);
    }
    event.preventDefault();
