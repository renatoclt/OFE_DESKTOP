...
var oOcCompradorBuscarComponent;
var datatable;
@Component({
  moduleId: module.id,
  selector: 'occompradorbuscar-cmp',
  templateUrl: 'occompradorbuscar.component.html',
  providers: [MasterService, LoginService]
})

export class OcCompradorBuscarComponent implements OnInit, AfterViewInit {
  util: AppUtils;
    ...
    if (botones) {
      ...
      this._securityService.obtenerBotones(this.url_main_module_page).subscribe(
        botones => {
          ...
          oOcCompradorBuscarComponent.configurarBotones(botones);
          oOcCompradorBuscarComponent._securityService.guardarBotonesLocalStore(this.url_main_module_page, botones);
        },
        e => console.log(e),
        () => { });
  ...
  validarfiltros() {
    ...
    if (this.filtro.fechacreacioninicio != null && this.filtro.fechacreacioninicio.toString() != "" && this.filtro.fechacreacionfin != null && this.filtro.fechacreacionfin.toString() != "") {
      let fechacreacioninicio = DatatableFunctions.ConvertStringToDatetime(oOcCompradorBuscarComponent.filtro.fechacreacioninicio);
      let fechacreacionfin = DatatableFunctions.ConvertStringToDatetime(oOcCompradorBuscarComponent.filtro.fechacreacionfin);
  ...
  ngOnInit() {
    ...
    oOcCompradorBuscarComponent = this;

    this.util.listEstadoOC(function (data: ComboItem[]) {
      oOcCompradorBuscarComponent.listEstadoCombo = data;
    });
    this.filtroDefecto();
...
function filtrarResultados(item) {
  ...
  let nroordencomprafiltro = oOcCompradorBuscarComponent.filtro.nroordencompra as string;
...
function cargarDataTable() {
  ...
  datatable = $('#dtResultados').on('init.dt', function (e, settings, json) {
    ...
    ajax: {
      ...
      beforeSend: function (request) {
        if (!oOcCompradorBuscarComponent.util.tokenValid()) {
          return;
        };
      ...
      data: function (d) {
        ...
        if (oOcCompradorBuscarComponent.filtro.ruc != "") {
          d.ruc = oOcCompradorBuscarComponent.filtro.ruc.trim();
        }

        if (oOcCompradorBuscarComponent.filtro.razonsocial != "") {
          d.razonsocial = oOcCompradorBuscarComponent.filtro.razonsocial.trim();

        }



        if (oOcCompradorBuscarComponent.filtro.fechacreacioninicio) {

          let fechacreacioninicio = DatatableFunctions.ConvertStringToDatetime(oOcCompradorBuscarComponent.filtro.fechacreacioninicio);
          d.FechaRegistroInicio = DatatableFunctions.FormatDatetimeForMicroService(fechacreacioninicio);
        }

        if (oOcCompradorBuscarComponent.filtro.fechacreacionfin) {

          let fechacreacionfin = DatatableFunctions.AddDayEndDatetime(DatatableFunctions.ConvertStringToDatetime(oOcCompradorBuscarComponent.filtro.fechacreacionfin));
          d.FechaRegistroFin = DatatableFunctions.FormatDatetimeForMicroService(fechacreacionfin);
        }

        let tipos_oc = [];

        if (oOcCompradorBuscarComponent.filtro.material)
          tipos_oc.push('M');
        if (oOcCompradorBuscarComponent.filtro.servicio)
          tipos_oc.push('S');
  ...
  datatable.on('click', '.edit', function (event) {
    if (oOcCompradorBuscarComponent.botonDetalle.habilitado) {
      ...
      //console.log("click edit", oOcCompradorBuscarComponent);
      let nav = ['/ordencompra/proveedor/formulario', nroordencompra];

      oOcCompradorBuscarComponent.navigate(nav);
    }
    event.preventDefault();
