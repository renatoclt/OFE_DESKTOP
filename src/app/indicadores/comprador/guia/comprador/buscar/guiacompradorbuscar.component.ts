...
import { URL_BUSCAR_GUIAREPORTE } from 'app/utils/app.constants';
...
var oGuiaCompradorBuscarComponent;
var datatable;
@Component({
  moduleId: module.id,
...
export class GuiaCompradorBuscarComponent implements OnInit, AfterViewInit {
    ...
    if (botones) {
      ...
      this._securityService.obtenerBotones(this.url_main_module_page).subscribe(
        botones => {
          ...
          oGuiaCompradorBuscarComponent.configurarBotones(botones);
          oGuiaCompradorBuscarComponent._securityService.guardarBotonesLocalStore(this.url_main_module_page, botones);
        },
        e => console.log(e),
        () => { });
  ...
  validarfiltros() {
    ...
    if (this.filtro.fechacreacioninicio != null && this.filtro.fechacreacioninicio.toString() != "" && this.filtro.fechacreacionfin != null && this.filtro.fechacreacionfin.toString() != "") {
      let fechacreacioninicio = DatatableFunctions.ConvertStringToDatetime(oGuiaCompradorBuscarComponent.filtro.fechacreacioninicio);
      let fechacreacionfin = DatatableFunctions.ConvertStringToDatetime(oGuiaCompradorBuscarComponent.filtro.fechacreacionfin);
  ...
  ngOnInit() {
    ...
    oGuiaCompradorBuscarComponent = this;

    this.util.listEstadoOC(function (data: ComboItem[]) {
      oGuiaCompradorBuscarComponent.listEstadoCombo = data;
    });
    this.filtroDefecto();
...
function filtrarResultados(item) {
  ...
  let nroordencomprafiltro = oGuiaCompradorBuscarComponent.filtro.nroordencompra as string;
...
function cargarDataTable() {
  ...
  datatable = $('#dtResultados').on('init.dt', function (e, settings, json) {
    ...
    ajax: {
      ...
      beforeSend: function (request) {
        if (!oGuiaCompradorBuscarComponent.util.tokenValid()) {
          return;
        };
      ...
      url: URL_BUSCAR_GUIAREPORTE,
      dataSrc: "data",
      data: function (d) {
        console.log(d)

        if (oGuiaCompradorBuscarComponent.filtro.ruc != "") {
          d.ruc = oGuiaCompradorBuscarComponent.filtro.ruc.trim();
        }

        if (oGuiaCompradorBuscarComponent.filtro.razonsocial != "") {
          d.razonsocial = oGuiaCompradorBuscarComponent.filtro.razonsocial.trim();

        }



        if (oGuiaCompradorBuscarComponent.filtro.fechacreacioninicio) {

          let fechacreacioninicio = DatatableFunctions.ConvertStringToDatetime(oGuiaCompradorBuscarComponent.filtro.fechacreacioninicio);
          d.FechaRegistroInicio = DatatableFunctions.FormatDatetimeForMicroService(fechacreacioninicio);
        }

        if (oGuiaCompradorBuscarComponent.filtro.fechacreacionfin) {

          let fechacreacionfin = DatatableFunctions.AddDayEndDatetime(DatatableFunctions.ConvertStringToDatetime(oGuiaCompradorBuscarComponent.filtro.fechacreacionfin));
          d.FechaRegistroFin = DatatableFunctions.FormatDatetimeForMicroService(fechacreacionfin);
        }

        let tipos_oc = [];

        if (oGuiaCompradorBuscarComponent.filtro.material)
          tipos_oc.push('M');
        if (oGuiaCompradorBuscarComponent.filtro.servicio)
          tipos_oc.push('S');
    ...
    columns: [
      ...
      { data: 'RucProveedor', name: 'Proveedor' },
      { data: 'RazonSocialProveedor', name: 'NombreProveedor' },
  ...
  datatable.on('click', '.edit', function (event) {
    if (oGuiaCompradorBuscarComponent.botonDetalle.habilitado) {
      ...
      //console.log("click edit", oGuiaCompradorBuscarComponent);
      let nav = ['/ordencompra/proveedor/formulario', nroordencompra];

      oGuiaCompradorBuscarComponent.navigate(nav);
    }
    event.preventDefault();
