...
import { reporteFiltros, reporteBuscar } from 'app/model/indicadoreshas';

import { AppUtils } from 'app/utils/app.utils';
import { MasterService } from 'app/service/masterservice';
import { ComboItem } from 'app/model/comboitem';
import { URL_BUSCAR_FACTURA } from 'app/utils/app.constants';
...
var oFacturaCompradorBuscarComponent;
var datatable;
...
export class FacturaCompradorBuscarComponent implements OnInit, AfterViewInit {
  ...
  obtenerBotones() {
    const botones = this._securityService.ObtenerBotonesCache(this.url_main_module_page) as Boton[];
    if (botones) {
      ...
      this._securityService.obtenerBotones(this.url_main_module_page).subscribe(
        botones => {
          ...
          oFacturaCompradorBuscarComponent.configurarBotones(botones);
          oFacturaCompradorBuscarComponent._securityService.guardarBotonesLocalStore(this.url_main_module_page, botones);
        },
        e => console.log(e),
        () => { });
    }
  ...
  validarfiltros() {
    ...
    if (this.filtro.fechacreacioninicio != null && this.filtro.fechacreacioninicio.toString() !== '' &&
        this.filtro.fechacreacionfin != null && this.filtro.fechacreacionfin.toString() !== '') {
      let fechacreacioninicio = DatatableFunctions.ConvertStringToDatetime(oFacturaCompradorBuscarComponent.filtro.fechacreacioninicio);
      let fechacreacionfin = DatatableFunctions.ConvertStringToDatetime(oFacturaCompradorBuscarComponent.filtro.fechacreacionfin);
  ...
  ngOnInit() {
    oFacturaCompradorBuscarComponent = this;

    this.util.listEstadoOC(function (data: ComboItem[]) {
      oFacturaCompradorBuscarComponent.listEstadoCombo = data;
    });
    this.filtroDefecto();
...
function filtrarResultados(item) {
    ...
    let nroordencomprafiltro = oFacturaCompradorBuscarComponent.filtro.nroordencompra as string;
...
function cargarDataTable() {
  ...
  datatable = $('#dtResultados').on('init.dt', function (e, settings, json) {
    ...
    ajax: {
      ...
      beforeSend: function (request) {
        if (!oFacturaCompradorBuscarComponent.util.tokenValid()) {
          return;
        };
        request.setRequestHeader('Authorization', 'Bearer ' + localStorage.getItem('access_token'));
        request.setRequestHeader('origen_datos', 'PEB2M');
        request.setRequestHeader('tipo_empresa', localStorage.getItem('tipo_empresa'));
      ...
      url: URL_BUSCAR_FACTURA,
      dataSrc: 'data',
      data: function (d) {
        console.log(d)

        if (oFacturaCompradorBuscarComponent.filtro.ruc != "") {
          d.ruc = oFacturaCompradorBuscarComponent.filtro.ruc.trim();
        }

        if (oFacturaCompradorBuscarComponent.filtro.razonsocial != "") {
          d.razonsocial = oFacturaCompradorBuscarComponent.filtro.razonsocial.trim();

        }



        if (oFacturaCompradorBuscarComponent.filtro.fechacreacioninicio) {

          let fechacreacioninicio = DatatableFunctions.ConvertStringToDatetime(oFacturaCompradorBuscarComponent.filtro.fechacreacioninicio);
          d.FechaRegistroInicio = DatatableFunctions.FormatDatetimeForMicroService(fechacreacioninicio);
        }

        if (oFacturaCompradorBuscarComponent.filtro.fechacreacionfin) {

          let fechacreacionfin = DatatableFunctions.AddDayEndDatetime(DatatableFunctions.ConvertStringToDatetime(oFacturaCompradorBuscarComponent.filtro.fechacreacionfin));
          d.FechaRegistroFin = DatatableFunctions.FormatDatetimeForMicroService(fechacreacionfin);
        }

        let tipos_oc = [];

        if (oFacturaCompradorBuscarComponent.filtro.material) {
          tipos_oc.push('M');
        }
        if (oFacturaCompradorBuscarComponent.filtro.servicio) {
          tipos_oc.push('S');
        }
    ...
    columns: [
      ...
      { data: 'NITVendedor', name: 'Proveedor' },
      { data: 'RazonSocialCliente', name: 'NombreProveedor' },
      { data: 'Emitidas', name: 'Emitidas' },
      { data: 'Publicadas', name: 'Publicadas' },
      { data: 'Uso', name: 'Porcentaje' }
  ...
  datatable.on('click', '.edit', function (event) {
    if (oFacturaCompradorBuscarComponent.botonDetalle.habilitado) {
      ...
      // console.log("click edit", oFacturaCompradorBuscarComponent);
      const nav = ['/ordencompra/proveedor/formulario', nroordencompra];

      oFacturaCompradorBuscarComponent.navigate(nav);
    }
    event.preventDefault();
