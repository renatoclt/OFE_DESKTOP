...
export class FacturaProveedorBuscarComponent implements OnInit, AfterViewInit {
  ...
  constructor(private cpService: FacturaService, private router: Router,
  ...
  validarfiltros() {
    ...
    if(this.filtro.fechaemisioninicio.toString() === '' ||  this.filtro.fechaemisionfin.toString() === ''){
      swal({
        html: '<p class="text-center">Debe ingresar un rango de fechas<p>',
        type: 'warning',
    ...
    if (this.filtro.fechaemisioninicio != null && this.filtro.fechaemisioninicio.toString() !== '' &&
        this.filtro.fechaemisionfin != null && this.filtro.fechaemisionfin.toString() !== '') {
  ...
  async  DescartarBorradores(event) {
      ...
      const checkboxGuias = $('#dtResultados').find('.checkboxCP:checked');
      ...
      ajax: {
        ...
        data: function (d) {
          ...
          if (oFacturaBuscarComponent.filtro.fechaemisioninicio) {
            const fechacreacioninicio = DatatableFunctions.ConvertStringToDatetime(oFacturaBuscarComponent.filtro.fechaemisioninicio);
          ...
          if (oFacturaBuscarComponent.filtro.fechaemisionfin) {
            const fechacreacionfin = DatatableFunctions.AddDayEndDatetime(DatatableFunctions.ConvertStringToDatetime(oFacturaBuscarComponent.filtro.fechaemisionfin));
            d.FechaEmision_fin = DatatableFunctions.FormatDatetimeForMicroService(fechacreacionfin);
          }


          d.column_names = 'IdComprobante,NumeroFactura,RazonSocialProveedor,TipoComprobante,DocumentoErp,FormaPago,Total,Moneda,Estado,FechaEmision,FechaProgramadaPago,FechaPago,Observaciones,ObservacionesPago,RazonSocialCliente,MotivoRechazoSAP,MotivoErrorSAP';
...
*/          
        ...
        },
        {
          render: function (data, type, row) {
            ...
            let estadotag = row.Estado
            if (estado == "Pendiente de aprobación"){
              return '<a class="editar" href="">' + row.Estado + '</a>'
            }
            return estadotag
          },
          targets: 7
...

        {
          ...
          render: function (data, type, row) {
            ...
            let titleFac = "Factoring"
            let title = "Ver/Editar";
            if (oFacturaBuscarComponent.filtro.publicada)
              title = "Ver";
            //return data +' ('+ row[3]+')';
            let showFac = 'not-show'
            let disabled = '';
            ...
            return '<div class="text-center"><a class="editar" ' + href + ' row-id="' + row.IdComprobante + '" esBorrador="' + esBorrador + '" id_doc="' + row.id_doc + '">' +
              ...
              '<a class="editar" ' + href + ' row-id="' + row.IdComprobante + '" esBorrador="' + esBorrador + '" id_doc="' + row.id_doc + '">' +
              '<button class="btn btn-simple color-green btn-info btn-icon edit '+ showFac +'" rel="tooltip" title="' + titleFac + '" data-placement="left"><i class="material-icons">assessment</i></button></a>' +
              '</div>';
          },
          targets: 12
        },
