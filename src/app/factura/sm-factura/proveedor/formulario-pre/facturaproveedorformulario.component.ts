...
import { DetalleFactura, Factura, ClienteFiltros, GuiaFiltros } from 'app/model/sm-factura';
...
import { FacturaService } from 'app/service/sm-facturaservice';
...
import { URL_BUSCAR_GUIA, BASE_URL, URL_BUSCAR_HAS, URL_BUSCAR_ORGANIZACION } from 'app/utils/app.constants';
...
var oFacturaProveedorFormularioComponent: FacturaProveedorFormularioComponentPre, dtArticulos, dtArticulosHAS, dtClientes, dtGuias, dtBuscarAceptacionServicio, dtArchivos, archivo: Archivo;
...
export class FacturaProveedorFormularioComponentPre implements OnInit, OnChanges, AfterViewInit {
  ...
  public url_main_module_page = '/sm-factura/proveedor/buscar';
  ...
  constructor(private router: Router, private route: ActivatedRoute, private _masterService: MasterService,
              private _dataServiceAdjunto: AdjuntoService, private _dataService: FacturaService,
              private _dataGuiaService: GuiaService, private _dataHASService: ConformidadServicioService,
...
*/
    ...
    dtArchivos.on('click', '.remove', function (e) {
        ...
        const row_id = $tr.find("a.editar").attr('row-id') as number;
...
",
        ...
        const regex2 = /^[0-9]+$/i;
        const regex3 = /^[a-z]+$/i;
        const str1 = oFacturaProveedorFormularioComponent.factura.nocomprobantepago1.trim();
...
",
    ...
    $("#nocomprobantepago2").blur(function () {
      ...
      if (oFacturaProveedorFormularioComponent.factura.esfisico) {
        const regex = /^[0-9]+$/i;
        const str = oFacturaProveedorFormularioComponent.factura.nocomprobantepago2.trim();
        ...
        const regex1 = /^[a-z0-9]+$/i;
        const regex2 = /^[0-9]+$/i;
        const str2 = oFacturaProveedorFormularioComponent.factura.nocomprobantepago2.trim();
...
",
          ...
          oFacturaProveedorFormularioComponent._dataService.descartarBorrador(oFacturaProveedorFormularioComponent.id)
            ...
            p => {
              swal({
                ...
                function () {
                  let nav = ['/sm-factura/proveedor/buscar'];
  ...
  eliminarArchivos(event) {
    event.preventDefault();
    const checkboxes = $('#dtArchivos').find('.checkboxArchivos:checked');
  ...
  async validardatos(e, enviar = false) {
      ...
      if ( this.factura.eselectronico && ( ( numeral(this.factura.fise).value()==null || numeral(this.factura.fise).value()==0 ) && ( numeral(this.factura.montosunat).value()!=numeral(this.factura.importetotal).value() ) ) ) {
          swal({
        ...
        */
        {
            render: function (data, type, row) {
                return '<div class="checkbox text-right"><label>'+
                      '<input name="checkArticuloGuia" type="checkbox" checked  value="' + row.IdProdxGuia + '" class=" checkbox-igv-articulo">'+
                      '</label></div>';
            },
            targets: 8
        },
        {
          ...
          //    alert(row.importetotalitem);
                ...
                return '<input style="width:90px;" name="igv-importe-articulo-guia-'+row.IdProdxGuia+'" type="text" class="form-control" monto-igv-item="'+valor+'" value="'+valor+'" disabled>';
            },
            targets: 9,
        },
        {
            render: function (data, type, row) {
                return '<input style="width:90px;" name="isc-importe-articulo-guia-'+row.IdProdxGuia+'" type="text" class="importeIscArticulosGuia form-control"  value="">';
            },
            targets: 10,
        },
    ]
