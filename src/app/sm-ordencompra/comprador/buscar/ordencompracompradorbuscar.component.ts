...
import { OrdenCompraBuscar, OrdenCompraFiltros } from '../../../model/sm-ordencompra';
...
export class OrdenCompraCompradorBuscarComponent implements OnInit, AfterViewInit {
  ...
  public url_main_module_page = '/sm-ordencompra/comprador/buscar';
  public navigate(nav) {
...
function cargarDataTable() {
  ...
  datatable = $('#dtResultados').on('draw.dt', function (e, settings, json) {
    ...
    columnDefs: [
      ...
      },
      {
        render: function (data, type, row) {
          let disabled = '';
          let href = 'href="/sm-ordencompra/comprador/formulario/' + row.CodigoOrden + '"';
  ...
  datatable.on('click', '.edit', function (event) {
    ...
    if (oOrdenCompraCompradorBuscarComponent.botonDetalle.habilitado) {
      ...
      let nav = ['/sm-ordencompra/comprador/formulario', nroordencompra];
