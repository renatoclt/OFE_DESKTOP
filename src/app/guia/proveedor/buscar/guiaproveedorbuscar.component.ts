...
function cargarDataTable() {
      ...
      },
      {
        render: function (data, type, row) {
          ...
          if (oGuiaBuscarComponent.filtro.publicada)
            title = "Ver";
          //return data +' ('+ row[3]+')';
          return '<div class="text-center"><a href="/guia/proveedor/formulario/' + row.CodigoGuia + '" row-id="' + row.CodigoGuia + '" esBorrador="' + esBorrador + '" id_doc="' + row.id_doc + '">' +
  ...
  datatable.on('click', '.edit', function (event) {
    ...
    let nav = ['/guia/proveedor/formulario', row_id, { b: esBorrador, c: id_doc }];
